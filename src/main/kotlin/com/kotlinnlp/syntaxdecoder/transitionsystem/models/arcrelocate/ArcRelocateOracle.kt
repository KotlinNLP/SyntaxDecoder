 ...
 */
open class ArcRelocateOracle(goldDependencyTree: DependencyTree)
  : Oracle<StackBufferState, ArcRelocateTransition>(goldDependencyTree) {

  /**
   * The OracleFactory.
   */
  companion object Factory : OracleFactory<StackBufferState, ArcRelocateTransition> {
   ...
   */
  override fun copy(): Oracle<StackBufferState, ArcRelocateTransition> {

    val clone = ArcRelocateOracle(this.goldDependencyTree)
   ...
   */
  override fun cost(transition: ArcRelocateTransition): Int =
    when (transition) {
      ...
      is Wait -> transition.calculateCost()
